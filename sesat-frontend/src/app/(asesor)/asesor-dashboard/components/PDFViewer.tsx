...
const PDFViewer = ({
    ...
    try {
        ...
        return (
            <div className='w-full flex flex-col rounded h-fit'>
                ...
                <div className='text-[#5f5f5f] px-3 flex items-center w-full border border-solid border-slate-300/80 h-[50px] bg-[#eeeeee]'>
                    ...
                    <div className='w-fit mx-auto'>
                        <input min={1} max={numPages} type="number" onChange={handleOnChangeInput} onKeyDown={handleKeyDown} placeholder='pág.' className='w-[80px] rounded border-[#5f5f5f]/30 border border-solid px-3 text-center text-[14px] py-1' />
                        <span className='text-[14px] ml-3'>{`/ ${numPages}`}</span>
                    </div>

                    {/**Descargar */}
                    <a href={blobURL} download="filename.pdf">
                        <div className='ml-3 w-[22px] cursor-pointer'>
                            <svg stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" height="22px" width="22px" xmlns="http://www.w3.org/2000/svg"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>
                        </div>
                    </a>
                ...
                {/**Document Container */}
                <div className='w-full border-t-0 border border-solid border-slate-300/80 h-full overflow-y-hidden'>
                    <center>

                        <div className="z-10 h-screen overflow-y-scroll bg-slate-100 ">
