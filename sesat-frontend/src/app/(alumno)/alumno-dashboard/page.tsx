...
export default async function Home() {
  ...
  let asignaciones: AsignacionProps[] = await findAsignacionesByPeriodAndAlumno(
    periodo.id_periodo,
    alumnoID,
    token
  );
  ...
  let contactos: Usuario[] = await findContactsByIdTesis(tesis.id_tesis, token);
          ...
          {tesis && tesis.fecha_registro === null ? (
            <div className="bg-[#ffffff] gray__border w-full p-6 overflow-x-scroll lg:overflow-hidden">
              <div className="w-full py-2">
                <p className="text-xl font-SESAT">
                  Realiza tu registro de Tesis para acceder a las asignaciones
                </p>
              </div>
              <div className="w-full py-2 flex gap-2 justify-end px-4">
                <div className="font-[14px] italic">
                  No te preocupes, el{" "}
                  <span className="font-semibold text-dark-blue-10">
                    título
                  </span>{" "}
                  y el{" "}
                  <span className="font-semibold text-dark-blue-10">
                  ...
                  pueden ser modificados posteriormente si es necesario.
                </div>
                <Link
                  className="primary__btn text-center"
                  href={"/alumno-dashboard/tesis-registry"}
                >
                  Realizar Registro
                </Link>
              </div>
