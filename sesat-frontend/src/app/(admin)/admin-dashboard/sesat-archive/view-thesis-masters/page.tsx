...
import { fetchTesisCompletadasMdHalfTimePaginated } from "../../../../../../utils/tesis.endpoint";
import { getTesisCompletadasMdHalfTimeByName } from "../../../../../../utils/tesis.endpoint";
import { fetchTesisCompletadasMaestriaMedTiempo } from "../../../../../../utils/tesis.endpoint";
...
export default async function Home({
  ...
  const search =
    ...
    if (search) {
      const tesisListData: Promise<InactiveTesisProps[]> = getTesisCompletadasMdHalfTimeByName(token, search);
      const tesisList = await tesisListData;
      const isDataEmpty = !Array.isArray(tesisList) || tesisList.length < 1 || !tesisList; //?
      return (
        <>
          <Search url={"/admin-dashboard/sesat-archive/view-thesis-masters"} />
              ...
              <div className="flex flex-col w-full">
                {tesisList?.map((tesis) => (
                  <TesisRow
                    ...
                    weblink={`view-thesis-masters/${tesis.id_tesis}`}
                  />
                ))}
              </div>
            ) : (
      ...
      const tesisListData: Promise<InactiveTesisProps[]> = fetchTesisCompletadasMdHalfTimePaginated(token, page, limit);
      const tesisList = await tesisListData;
      const isDataEmpty = !Array.isArray(tesisList) || tesisList.length < 1 || !tesisList; //?
  
      const totalTesisListData: Promise<InactiveTesisProps[]> = fetchTesisCompletadasMaestriaMedTiempo(token);
          ...
          <div className="mt-2 w-full flex justify-end">
            {!search  && !isDataEmpty? (
              <>
                <Link
                  href={{
                    pathname: "/admin-dashboard/sesat-archive/view-thesis-masters",
                    query: {
