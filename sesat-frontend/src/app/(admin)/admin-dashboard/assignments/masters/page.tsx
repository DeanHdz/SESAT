...
import {
  fetchNumAsignacionesPendientesDoctorado,
  fetchNumAsignacionesPendientesMaestriaMedioTiempo,
  fetchNumAsignacionesPendientesMaestriaTiempoComp,
} from "../../../../../../utils/asignacion.endpoint";
import {
  fetchNumAlumnosDoctorado,
  fetchNumAlumnosMaestriaMedTiempo,
  fetchNumAlumnosMaestriaTiempoComp,
} from "../../../../../../utils/tesis.endpoint";
...
async function fetchStatusFullTime(
  alumnos: Array<AvanceProps>
  ...
  const promises = alumnos?.map(async (elem, i) => {
    if (elem.count != "0") {
      //no disponible
      await fetchNumAsignacionesPendientesMaestriaTiempoComp(
        elem.t_ultimo_avance.toString(),
        ""
      ).then((result) => {
...
//Cuales asignaciones estan activas y cuales pendientes?
async function fetchStatusMidTime(
  alumnos: Array<AvanceProps>
): Promise<number[] | undefined> {
  ...
  const promises = alumnos?.map(async (elem, i) => {
    if (elem.count != "0") {
      //no disponible
      await fetchNumAsignacionesPendientesMaestriaMedioTiempo(
        elem.t_ultimo_avance.toString(),
        ""
      ).then((result) => {
        let total = parseInt(result);
...
export default async function Home() {
  ...
  const statusFullTime = await fetchStatusFullTime(
    await fetchAsignacionesDataFullTime()
  );
  const statusMidTime = await fetchStatusMidTime(
    await fetchAsignacionesDataMidTime()
  );
    ...
    <main>
      <div className="w-full flex flex-col mb-10">
        ...
        <label className=" block text-xl font-bold">
          Avances de tesis para este semestre
        </label>
