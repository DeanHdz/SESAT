...
const AddEventModal = ({startDate, endDate, isClicked}:{startDate: Date, endDate: Date, isClicked: boolean}) => {
        ...
        <dialog id="add_event_modal" className="modal">
            <div className="modal-box max-w-[400px]">
                <div className="w-full flex flex-row h-fit">
                    ...
                    {!isClicked && (
                        <>
                            <label className="mx-auto  flex items-center font-thin text-sm">hasta</label>
                            <div>
                                <div className="font-bold text-3xl">{endDate?.getDate().toLocaleString()}</div>
                                <div className="font-light text-sm">{days[endDate?.getDay()!]}</div>
                            </div>
                        </>
                    )}
                </div>
                ...
                <div className="modal-action">
                    ...
                    <form method="submit" className="flex flex-col w-full">
                        ...
                        <div className="w-full flex justify-end mt-3">                            
                            <button className="primary__btn" type='submit'>Añadir</button>
                        </div>
                    </form>
                </div>
            </div>
