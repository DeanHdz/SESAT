...
const StudentProfileModal = ({ user }: { user: Usuario }) => {
  ...
  const handleDedicationChange = async () => {
    ...
    if(res)
    {
      ...
      setShowDedicationSuccessModal(!showDedicationSuccessModal);
    }
    else{
      setShowDedicationModal(!showDedicationModal);
      setShowChangeStatusErrorModal(!showChangeStatusErrorModal);
          ...
          <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <div className="sm:flex sm:items-start">
                ...
                <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      La modalidad del alumno ha sido modificada con éxito.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
              <button
                ...
                className="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto"
                onClick={() => {
                  setShowDedicationSuccessModal(
                    !showDedicationSuccessModal
                  );
                  revalidator("PaginatedMastersList");
                  revalidator("PaginatedPhdList");
                  router.push(
  ...
  return (
    <>
      ...
      {showDedicationSuccessModal ? <>{dedicationSuccessModal}</> : ""}
      {showRevalidateStudentSuccessModal ? (
