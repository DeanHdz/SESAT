...
const DocumentView = () => {
  ...
  const id_assign: string = "102"; //location.state.id_assign;
  const pdfType: string =  "2"; //location.state.pdfType;
    ...
    async function requestTesis() {
      if (id_assign) {
        try {
          ...
          /*
          const resp = await TesisEndpoint.getTesis(parseInt(id_assign), "");
          if (resp) {
          }
          */
        } catch (error) {}
    ...
    async function requestActa() {
      if (id_assign) {
        try {
          ...
          console.log(pdfType);
          /*
          var resp = await ActaEvaluacionEndpoint.getActaEvaluacion(
            parseInt(id_assign),
            ""
          );
          if (resp) {
            ...
            decodeBase64Async(resp.documento_rellenado).then(
              (decodedBuffer) => {
